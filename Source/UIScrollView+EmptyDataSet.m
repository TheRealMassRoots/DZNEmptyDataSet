...
{
    if (self.emptyDataSetSource && [self.emptyDataSetSource respondsToSelector:@selector(buttonBackgroundForEmptyDataSet:forState:)]) {
        UIColor *color = [self.emptyDataSetSource buttonBackgroundForEmptyDataSet:self forState:state];
        if (color) NSAssert([color isKindOfClass:[UIColor class]], @"You must return a valid UIColor object -imageTintColorForEmptyDataSet:");
        return color;
    }
    return nil;
...
- (void)dzn_reloadEmptyDataSet
{
    ...
    if ([self dzn_shouldDisplay] && [self dzn_itemsCount] == 0)
    {
        ...
        if (customView) {
            ...
            UIColor *backgroundButtonColor = [self dzn_buttonBackgroundColorForState:UIControlStateNormal];
            ...
            if (buttonImage) {
                ...
                [view.button setBackgroundColor:backgroundButtonColor];
                [view.button.layer setCornerRadius:2];
                [view.button setClipsToBounds:YES];
